...
const IssueTracking: React.FC = () => {
  ...
  const [selectedTimerId, setSelectedTimerId] = useState<number | ''>('');

  const handleAddIssue = () => {
    if (newIssue.trim() !== '' && selectedTimerId !== '') {
      addIssue({ 
        id: Date.now(), 
        timerId: Number(selectedTimerId),
        text: newIssue, 
        status: 'open'
      });
      setNewIssue('');
    ...
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="mb-4">
        ...
        >
          ...
          {timers.map(timer => (
            <option key={timer.id} value={timer.id}>{timer.projectName}</option>
          ))}
        </select>
        <input
          type="text"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Timer: {timers.find(t => t.id === issue.timerId)?.projectName || 'Unknown'}
                </p>
              </div>
              <select
                value={issue.status}
