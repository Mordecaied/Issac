...
const TimeVisualization: React.FC = () => {
  ...
  const data = timers.map(timer => ({
    ...
    time: timer.time,
    subTimersTime: timer.subTimers.reduce((acc, subTimer) => acc + subTimer.time, 0),
  }));
    ...
    <div className="h-96 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          ...
          />
          <Legend />
          <Bar dataKey="time" fill="#8884d8" name="Main Timer" />
          <Bar dataKey="subTimersTime" fill="#82ca9d" name="Sub-timers" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
