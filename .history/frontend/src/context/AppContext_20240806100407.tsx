...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addTimer = (timer: Timer) => {
    setTimers([...timers, { ...timer, lastUpdated: new Date().toISOString() }]);
  };
  ...
  const deleteTimer = (id: number) => {
    ...
    setQuestions(questions.filter(question => question.timerId !== id));
    setIssues(issues.filter(issue => issue.timerId !== id));
  };
