...
import { useAppContext, Topic, SubTopic } from '../../contexts/AppContext';
  ...
  const handleAddNewTopic = () => {
    if (newTopicName.trim()) {
      const newTopic: Topic = {
        ...
        subTopics: [],
        questions: [],
        duration: 0, // Add this line to include the duration property
      };
      addTopic(newTopic);
      setNewTopicName('');
    }
      ...
      const updatedTopics = topics.map((topic) => {
        if (topic.id === topicId) {
          return {
            ...
            subTopics: [
              ...topic.subTopics,
              { id: Date.now().toString(), name: newSubTopicName.trim(), questions: [] },
            ],
          };
        }
        return topic;
          ...
          {({ open }) => (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
              ...
              <Disclosure.Panel className="px-4 pt-4 pb-2 text-sm text-gray-500 dark:text-gray-300">
                <div className="mb-4">
                  <h3 className="font-semibold text-lg mb-2">Topic Questions</h3>
                ...
                {topic.subTopics.map((subTopic: SubTopic) => (
                  <div key={subTopic.id} className="mb-4">
                    ...
                    <QuestionsList topicId={topic.id} subTopicId={subTopic.id} />
                  </div>
                ))}
                <div className="mt-4">
                  <input
